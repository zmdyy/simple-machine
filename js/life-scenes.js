...
(function (global) {
  ...
  const LAYOUT = {
    ...
    nailclipper(t) {
      // 用户新提供的完整实物图：仓库内使用 1200×882 PNG。
      // 图 2 是同源标注参考图，第一阶段 O / A(F₁) / B(F₂) 按标注像素精确换算。
      // 目标宽度 560 px，可完整落在 800×420 的 SVG 画布内。
      const x = 120;
      const y = 4;
      const w = 560;
      const h = w * (882 / 1200);
      ...
      if (t < 1 / 3) {
        ...
        const O = P(603 / 2048, 966 / 1505);    // 支点 O（绿色标注）
        const p1 = P(1323 / 2048, 244 / 1505);  // 动力作用点 A（红色标注）
        const p2 = P(561 / 2048, 915 / 1505);   // 阻力作用点 B（蓝色标注）
        return pack(O, p1, K.v(0, 1), p2, K.v(0, 1), [p2, O, p1], 20, {
          stageName: '① 压柄：第一类杠杆（省力）',
          caption: '指甲剪①压柄：按标注参考图，O 为前端支承点，A 为手向下压的动力点，B 为阻力作用点。',
        });
      }
  ...
  function draw(g, id, t, opts) {
    ...
    if (id === 'crowbar') {
      ...
      const bar = fit(
        g,
        'assets/life/nailclipper-photo.png?v=20260925p',
        1200, 882,
        120, 4,
        w
      );
      ...
      let O, p1, p2;
      if (t < 1 / 3) {
        O = bar.p(603 / 2048, 966 / 1505);
        p1 = bar.p(1323 / 2048, 244 / 1505);
        p2 = bar.p(561 / 2048, 915 / 1505);
      } else if (t < 2 / 3) {
        O = bar.p(0.920, 0.635);
